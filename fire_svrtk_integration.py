...
def process(connection, config, metadata):
    ...
    try:
        ...
        # voxel_dims = ((metadata.encoding[0].encodedSpace.fieldOfView_mm.x / metadata.encoding[0].encodedSpace.matrixSize.x),
...
def process_raw(group, connection, config, metadata):
    ...
    np.save(debugFolder + "/" + timestamp + "imgCrop.npy", data)
...
def process_image(images, connection, config, metadata):
    ...
    timestamp = f"{datetime.today().strftime('%H-%M-%S')}"
    np.save(debugFolder + "/" + timestamp + "-imgOrig.npy", data)
    ...
    currentSeries = 0
    # print(data.shape)
    im = np.zeros((data.shape[0], data.shape[1], data.shape[2], data.shape[3], data.shape[4]))
    print(im.shape)
    ...
    for iImg in range(data.shape[-1]):
        ...
        # from_array() should be called with 'transpose=False' to avoid warnings, and when called
        # with this option, can take input as: [cha z y x], [z y x], or [y x]
        imagesOut[iImg] = ismrmrd.Image.from_array(data[...,iImg].transpose((3, 2, 0, 1)), transpose=False)# Convert the 'Image' object to a NumPy array
        image_data = ismrmrd.Image.from_array(data[..., iImg], transpose=False).data

        # Assign the NumPy array to the 'im' array along the specified channel
        im[..., iImg] = image_data
    ...
    print("oldHeader", oldHeader)

    dim = oldHeader.matrix_size
    # header['dim'] = dim

    # pixel dimension = FOV / matrix size
    fov = (ctypes.c_float(oldHeader.field_of_view[0]),
           ctypes.c_float(oldHeader.field_of_view[1]),
           ctypes.c_float(oldHeader.field_of_view[2]))

    pixdim = np.array([oldHeader.field_of_view[1] / oldHeader.matrix_size[1],
           oldHeader.field_of_view[1] / oldHeader.matrix_size[1],
           oldHeader.field_of_view[2] / oldHeader.matrix_size[2]])

    header['pixdim'][1:4] = pixdim

    # rotation matrix of the object - read/phase/slice directions
    read = oldHeader.read_dir
    phase = oldHeader.phase_dir
    slice = oldHeader.slice_dir

    header['pixdim'][0] = read[2]
    header['pixdim'][6] = phase[0]
    header['pixdim'][7] = slice[1]

    pos = oldHeader.position
    ...
    srow_x = np.array([read[0], read[1], read[2], pos[0]])
    srow_y = np.array([phase[0], phase[1], phase[2], pos[1]])
    srow_z = np.array([slice[0], slice[1], slice[2], pos[2]])

    header['srow_x'] = srow_x
    header['srow_y'] = srow_y
    header['srow_z'] = srow_z

    if oldHeader.slice % 2 != 0:
        mid = int(oldHeader.slice / 2) + 1
    else:
        mid = int(oldHeader.slice / 2)
    print("This is the mid slice: ", mid)
    im_corr2a = im[:, :, 0:mid+1]
    im_corr2b = im[:, :, mid+1:]

    im_corr2ab = np.zeros(np.shape(im))

    im_corr2ab[:, :, ::2] = im_corr2a
    im_corr2ab[:, :, 1::2] = im_corr2b
    # print(data.shape)
    nifti = nib.Nifti1Image(im_corr2ab, affine=None, header=header)
    nifti.to_filename(debugFolder + "/" + timestamp + "-imgSaved.nii.gz")
